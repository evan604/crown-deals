...
class Actor {
    ...
    async exit(messageOrOptions, options = {}) {
        ...
        await (0, timeout_1.addTimeoutToPromise)(async () => {
            ...
            if (options.statusMessage != null) {
                ...
                await Promise.race([statusMessagePromise, (0, utils_1.sleep)(1)]);
            }
    ...
    async call(actorId, input, options = {}) {
        const timeout = options.timeout === 'inherit'
    ...
    async addWebhook(options) {
        (0, ow_1.default)(options, ow_1.default.object.exactShape({
            ...
            headersTemplate: ow_1.default.optional.string,
            description: ow_1.default.optional.string,
            ignoreSslErrors: ow_1.default.optional.boolean,
            doNotRetry: ow_1.default.optional.boolean,
            shouldInterpolateStrings: ow_1.default.optional.boolean,
            isApifyIntegration: ow_1.default.optional.boolean,
        }));
        if (!this.isAtHome()) {
    ...
    async getInput() {
        ...
        let input = rawInput;
        if (ow_1.default.isValid(rawInput, ow_1.default.object.nonEmpty) &&
            ...
            input = (0, input_secrets_1.decryptInputSecrets)({ input: rawInput, privateKey });
        }
        if (ow_1.default.isValid(input, ow_1.default.object.nonEmpty) && !Buffer.isBuffer(input)) {
    ...
    async inferDefaultsFromInputSchema(input) {
        ...
        const inputSchema = (0, input_schemas_js_1.readInputSchema)();
        // Don't emit warning if there is no input schema defined
        if (inputSchema === input_schemas_js_1.noActorInputSchemaDefinedMarker) {
            return input;
        }
        if (!inputSchema) {
            log_1.default.warning('Failed to find the input schema for the local run of this Actor. Your input will be missing fields that have default values set if they are missing from the input you are using.');
            return input;
        }
        const defaults = (0, input_schemas_js_1.getDefaultsFromInputSchema)(inputSchema);
